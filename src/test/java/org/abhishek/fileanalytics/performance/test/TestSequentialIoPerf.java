package org.abhishek.fileanalytics.performance.test;
...
public final class TestSequentialIoPerf {
	...
	public static final long FILE_SIZE = PAGE_SIZE * 200L * 150L;
	public static final String FILE_NAME = "./src/test/resources/scn_aaa.txt";
	...
	public static void main(final String[] arg) throws Exception {
		...
		for (final PerfTestCase testCase : testCases) {
			for (int i = 0; i < 10; i++) {
				...
				out.format("%s\twrite=%,d\tread=%,d mbs/sec\tbuffer=%d\n",
						testCase.getName(), bytesWrittenPerSec, bytesReadPerSec, (PAGE_SIZE * testCase.incrBufferSize));
			}
		}
	...
	public abstract static class PerfTestCase {
		...
		protected int incrBufferSize;

		public PerfTestCase(final String name, final int incrBufferSize) {
			this.name = name;
			this.incrBufferSize = incrBufferSize;
		}
	...
	private static PerfTestCase[] testCases = {
			new PerfTestCase("RandomAccessFile", 0) {
			...
			new PerfTestCase("BufferedStreamFile", 0) {
				public int testWrite(final String fileName) throws Exception {
					int checkSum = 0;
			...
			new PerfTestCase("BufferedChannelFile", 0) {
			...
			new PerfTestCase("MemoryMappedFile", 0) {
				@SuppressWarnings("resource")
				public int testWrite(final String fileName) throws Exception {
					FileChannel channel = new RandomAccessFile(fileName, "rw")
							.getChannel();
				...
				public int testRead(final String fileName) throws Exception {
					FileChannel channel = new RandomAccessFile(fileName, "rw")
							.getChannel();
					MappedByteBuffer buffer = channel.map(READ_ONLY, 0,
							Math.min(channel.size(), MAX_VALUE));
					int checkSum = 0;
